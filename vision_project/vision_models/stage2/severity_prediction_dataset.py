...
from vision_project.vision_models.stage2.helper import augment, tensorize
...
class SeverityPredictionDataset(Dataset):
    ...
    def _create_split(self, df):
        ...
        df['composite_key'] = df.apply(lambda row: '_'.join(str(row[var]) for var in self.balance_variables), axis=1)
        pre_augment = df
    ...
    def _prepare_data(self):
        ...
        self.train_df, self.val_df, self.test_df, self.split_data = tensorize(self._create_split(df, self.balance_variables))
